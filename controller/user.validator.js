...
const userValidator = Joi.object({
    firstname: Joi.string().alphanum().min(3).max(25).required().error(errors => {
      errors.forEach(err => {
        switch (err.code) {
          case "string.empty":
    ...
    lastname: Joi.string().alphanum().min(3).max(25).required().error(errors => {
      errors.forEach(err => {
        switch (err.code) {
          case "string.empty":
    ...
    username: Joi.string().alphanum().min(3).max(25).required().error(errors => {
      errors.forEach(err => {
        switch (err.code) {
          case "string.empty":
            err.message = "User name should not be empty!";
            break;
          case "string.min":
            err.message = `User name should have at least ${err.local.limit} characters!`;
            break;
          case "string.max":
            err.message = `User name should have at most ${err.local.limit} characters!`;
            break;
          default:
            break;
        }
